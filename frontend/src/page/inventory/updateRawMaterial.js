...
const RawMaterialUpdateForm = () => {
  ...
  useEffect(() => {
    const fetchRawMaterialData = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/rawMaterial/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/rawMaterial/updateRawMaterial/${id}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        // Redirect to the raw material list page using navigate
        navigate("/inventory/rawMaterialList"); // Use navigate instead of history.push
      } else {
